-- ─────────────────────────────────────────────────────────────────────────────
-- Migration: Hotfix — idempotent recreation of stock transfer RPC functions
-- ─────────────────────────────────────────────────────────────────────────────
-- This migration is safe to apply multiple times (idempotent).
-- It ensures the following objects exist even when the schema cache is stale
-- or a previous migration was not fully applied:
...
-- stock_movements: employees see movements for themselves, admins/managers see all.
...
BEGIN
  ...
  INSERT INTO public.stock_commercial (user_id, category, quantity)
    ...
    DO UPDATE SET
      quantity   = public.stock_commercial.quantity + p_quantity,
      updated_at = now();
